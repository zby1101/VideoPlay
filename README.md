...
基于Qt、FFmpeg、SDL2 和 QOpenGLWidget 的视频播放器。项目支持本地视频文件播放、网络视频流播放、音频播放，并可以在 SDL2 和 OpenGL 两种绘制方式之间切换。
...
- 使用 FFmpeg 解码音频，使用 libswresample 转换音频采样格式，并通过 SDL2 输出音频。
- 支持 SDL2 和 QOpenGLWidget 两种视频绘制方式。
- 可在 `视图 -> 渲染器` 菜单中切换绘制方式，切换前如果正在播放，切换后会继续播放。
- 支持播放、暂停、停止。
- 本地视频支持 `0.5x`、`0.75x`、`1.0x`、`1.25x`、`1.5x`、`2.0x` 倍速播放。
- 本地视频支持左右方向键快退、快进。
- 支持右键菜单进入或退出全屏，`Esc` 可退出全屏。
- 状态栏显示文件名、分辨率、帧率和解码器信息。
- 网络流总时长未知时，界面使用 `-` 表示未知时长。
- 快进、快退等 seek 操作由解码线程处理，减少 FFmpeg 上下文并发访问风险。
- 有音频流时以音频播放时钟作为主时钟同步视频，视频明显落后时会丢弃迟到帧。
...
3. 使用 `播放/暂停` 按钮控制播放状态。
4. 使用 `停止` 按钮停止播放并清空画面。
5. 使用底部控制条的倍速下拉框切换本地视频播放速度。
6. 在 `视图 -> 渲染器` 菜单中选择 `SDL2 渲染` 或 `OpenGL 渲染` 切换渲染方式。
7. 播放本地视频时，可使用左方向键快退、右方向键快进。
8. 在视频区域右键可进入或退出全屏。
9. 全屏状态下按 `Esc` 退出全屏。
